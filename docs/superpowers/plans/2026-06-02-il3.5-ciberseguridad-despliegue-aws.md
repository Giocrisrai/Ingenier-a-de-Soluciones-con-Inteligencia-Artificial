...
> ⚠️ **DOCUMENTO HISTÓRICO — PARCIALMENTE OBSOLETO (2026-06-02).**
>
> Este plan describe la implementación tal como se hizo en su momento, **sobre GitHub Models**.
> El curso migró después a **Groq**, porque GitHub Models dejó de ofrecer capa gratuita.
>
> Los fragmentos de código, variables de entorno y comandos que aparecen más abajo
> (`GITHUB_TOKEN`, `OPENAI_BASE_URL`, `gpt-4o-mini`, `OpenAI SDK`) **ya no son válidos**.
> No los sigas al pie de la letra. La equivalencia actual es:
>
> | En este plan | Hoy |
> |---|---|
> | `GITHUB_TOKEN` | `GROQ_API_KEY` (obtenida en https://console.groq.com/) |
> | `OPENAI_BASE_URL` | *(no existe: el SDK de Groq ya apunta a su endpoint)* |
> | `AGENT_MODEL="gpt-4o-mini"` | `AGENT_MODEL="llama-3.1-8b-instant"` |
> | `from openai import OpenAI` | `from groq import Groq` |
>
> **La fuente de verdad actual es el código de `deploy/`, su `deploy/.env.example`,
> `deploy/README.md` y la guía `RA3/IL3.5/1-deploy-aws-paso-a-paso.md`.**
> Se conserva este archivo como registro de la decisión de diseño, que sigue siendo válida:
> la arquitectura (Caddy → Streamlit → FastAPI), los guardrails y el modo demo no cambiaron.
