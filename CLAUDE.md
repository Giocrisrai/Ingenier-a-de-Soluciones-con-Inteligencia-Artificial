...
# Agente actual (LangChain 1.x sobre LangGraph). create_react_agent de LangGraph está deprecado.
from langchain.agents import create_agent
from langgraph.checkpoint.memory import InMemorySaver
agente = create_agent(model=llm, tools=tools, checkpointer=InMemorySaver())
```
...
- **LangChain classic (`AgentExecutor`)** → `GROQ_MODEL` (contrast notebooks only).
